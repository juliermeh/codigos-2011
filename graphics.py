...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        master.bind("<Key>", self._get_keys)
        self.arrow = None
        self.key = None
    ...
    def _get_keys(self, e):
        k = e.keysym
        if k in ['Up', 'Down', 'Left', 'Right']:
            self.arrow = k
        else:
            if k not in ['space', 'Return'] and e.char:
                k = e.char
            if (k.isalnum() and len(k) == 1) or k in ['space', 'Return'] or k in '[]{}!@#$%&*()-_=+;:.>,<\|/?':
                if k == 'space':
                    k = ' '
                if k == 'Return':
                    k = '\n'
    ...
    def pollArrow(self, trials=10):
        """ Poll for a key. """
        self.arrow = None
        cont = 0
        while self.arrow == None and cont < trials:
            self.update()
            if self.isClosed(): raise GraphicsError("getKey in closed window")
            cont += 1
            time.sleep(.01) # give up thread
    ...
    def getInput(self):
        ...
        self.arrow = None
        self.key = None
        while (self.mouseX == None or self.mouseY == None) and self.key == None and self.arrow == None:
            self.update()
            if self.isClosed(): raise GraphicsError("getMouse in closed window")
            time.sleep(.01) # give up thread
        if self.mouseX:
